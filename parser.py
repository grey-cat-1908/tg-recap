...
BAD_WORDS_PATTERNS = [
    ...
    r"\b(хуй|хуе|хуё|хуя|хер|хуи)\w*\b",
    r"\b(пизд|пизда|пиздец)\w*\b",
    r"\b(еб|ёб|еба|ёба|уеб)\w*\b",
    r"\b(сука|сучк)\w*\b",
]
...
def parse_data(data):
    messages = (
        data["chats"]["list"][0]["messages"] if "chats" in data else data["messages"]
    )
    ...
    for msg in messages:
        ...
        if msg.get("media_type") == "sticker":
            stickers += 1
        elif msg.get("media_type") == "video_message":
            videos += 1
